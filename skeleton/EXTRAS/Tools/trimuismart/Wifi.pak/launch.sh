...
kill_presenter() {
    # Robustly stop any running presenter. A naive killall can race with a
    # just-spawned background presenter that has not exec'd yet, leaving a
    # stale overlay painted on top of the next screen.
    i=0
...
show_message() {
    ...
    if [ "$seconds" = "forever" ]; then
        ...
        # give it a beat to actually start, so a subsequent kill can find it
        sleep 0.2
    else
...
main() {
    ...
    allowed_platforms="miyoomini my282 my355 tg5040 rg35xxplus trimuismart"
